...
function _appendUserNotesSection(mode, featureId, versionA, versionB, container) {
  ...
  // History container (loaded async)
  ...
  // Load existing notes
  _loadNotes(mode, featureId, versionA, versionB, historyEl);
  ...
  submitBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      nameInput.value = "";
      commentInput.value = "";
      _prependNote(body.note, historyEl);
    } catch (err) {
...
async function _loadNotes(mode, featureId, versionA, versionB, container) {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/notes?` + params.toString(), { cache: "no-store" });
    if (!res.ok) return;
    const body = await res.json();
    ...
    notes.forEach(note => _prependNote(note, container));
  } catch (_) {
    // Non-fatal: notes are optional
  }
}

function _prependNote(note, container) {
  const details = document.createElement("details");
  details.className = "user-note";
  // Default closed per spec §9.5
  ...
  // Insert at top of container (after heading if present)
